...
// simple for a low-literacy audience: one idea per step, a big emoji, a dot indicator,
// Skip + Next. Persisted so it never nags again; clearing the key (Me → "Show the intro
// again") re-enables it. Renders client-only (after the localStorage check) so it never
// appears in the prerendered HTML and causes no hydration mismatch.
export const GUIDE_KEY = 'kaarigar:guideSeen'

const STEPS = [
  { icon: '🧭', title: 'guide.s1Title', body: 'guide.s1Body' },
  { icon: '🎥', title: 'guide.s2Title', body: 'guide.s2Body' },
  { icon: '📴', title: 'guide.s3Title', body: 'guide.s3Body' },
] as const
  ...
  return (
    ...
    >
      <div className="guide">
        ...
        <div className="guide__icon" aria-hidden="true">
          {s.icon}
        </div>
