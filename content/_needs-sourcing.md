...
**Status (2026-07-25):** **ALL 27 lessons now carry oEmbed-verified, embeddable videos** (42 IDs total). L10 (business) previously text-only — now has 3 newly-sourced Hindi videos, each oEmbed-verified: `l10-estimating-and-quoting` → `pEIPdTMYNgA` (Simply Interiors, "How to Measure Square Feet of woodwork"); `l10-clients-and-tools` → `cNMQYjHmkY8` (Raoji Akash, furniture-business start) + `h1GNISWahck` (Career Bhai, "Carpenter Job Work & Responsibility") — the earlier `YErp2_snJ4M` (Basant Art tools) was replaced on human review. ⚠️ **These 3 are NOT yet human-watched** — oEmbed proves they exist/embed, never that they teach the topic well; a human must watch each before `reviewStatus` → `expert-reviewed` (recall the rejected `wCbrDTd6mNM`). Each verified twice (sourcing + independent re-check) and re-checked every build by `verify:videos`. **Most L1/L2/L6/L7/L8/L9 primaries are Hindi/Indian creators; L3–L5 joinery + a few others fall back to English (Paul Sellers, Matt Estlea, RobCosman, SIKANA) — language labelled honestly.** STILL OPEN: a human must **watch each video** to confirm teaching quality before `reviewStatus` → `expert-reviewed`; Hindi alternates still wanted for the English-primary lessons. (Original L0 notes below.)
